"""v7.1 Gated Spectral State LM: MLM training with gated perturbations.

Like v7 but with content-dependent gating on state updates.
Each token computes a gate based on its coherence with the current state,
controlling how much to retain vs overwrite.
"""
...
from src.wave_embedding_v7_1 import GatedSpectralStateLM, mask_tokens
...
def main():
    ...
    for epoch in range(cfg.num_epochs):
        ...
        with torch.no_grad():
            ...
            # Gate bias stats: mean sigmoid(gate_bias) per scale
            gb = torch.sigmoid(model.embedding.gate_bias.data.cpu())
            gb_str = " ".join(f"{gb[:, s].mean():.3f}" for s in range(cfg.num_scales))
            gsens = model.embedding.gate_sensitivity.item()

        log.info(
            "Epoch %3d/%d | loss=%.4f | PPL=%.0f | acc=%.1f%% | "
            "decay=[%s] | gate_μ=[%s] gsens=%.2f | temp=%.3f | %.1fs",
            epoch + 1, cfg.num_epochs, avg_loss, ppl, accuracy,
            decay_str, gb_str, gsens, model.temperature.abs().item(), elapsed,
        )
    ...
    save_path = f"checkpoints/wave_spectral_v7_1{suffix}.pt"
    torch.save({
